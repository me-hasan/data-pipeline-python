...
logger.info(f"imds_idxs script started at: {start_time}")
...
try:
    ...
    mysql_query = "SELECT * FROM IDXS"
    logger.info(f"Executing query on MySQL: {mysql_query}")
    idxs_data = pd.read_sql(mysql_query, mysql_engine)
    logger.info(f"Fetched {len(idxs_data)} records from MySQL")

    # Initialize MetaData without the bind argument
    metadata = MetaData()
    
    # Reflect the target table (imds_idxs) from the PostgreSQL database
    imds_idxs_table = Table('imds_idxs', metadata, autoload_with=postgresql_engine)
    
    # Prepare data for bulk insert
    insert_records = []
    
    # Construct records for insertion
    for _, data in idxs_data.iterrows():
        record = {
            'id': str(uuid.uuid4()),
            'idx_total_trades': data['IDXS_TOTAL_TRADES'],
            'idx_total_volume': data['IDXS_TOTAL_VOLUME'],
            'idx_total_value': data['IDXS_TOTAL_VALUE'],
            'idx_lm_date_time': data['IDXS_LM_DATE_TIME']
        }
    ...
    with Session() as session:
        for record in insert_records:
            ...
            existing_record = session.query(imds_idxs_table).filter_by(
                idx_total_trades=record['idx_total_trades'],
                idx_lm_date_time=record['idx_lm_date_time']
            ).first()
    
            # Insert new record if it doesn't exist
            if not existing_record:
                insert_stmt = imds_idxs_table.insert().values(record)
